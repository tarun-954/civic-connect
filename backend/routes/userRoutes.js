...
router.get('/notification-preferences', requireAuth, async (req, res) => {
  try {
    ...
    return res.status(500).json({ status: 'error', message: 'Failed to fetch notification preferences' });
  }
});
...
router.put('/notification-preferences', requireAuth, [
  ...
  body('reportUpdates').optional().isBoolean().withMessage('reportUpdates must be a boolean'),
  ...
  try {
    ...
    const updatePayload = {};
    if (pushEnabled !== undefined) updatePayload['notificationPreferences.pushEnabled'] = pushEnabled;
    if (reportUpdates !== undefined) updatePayload['notificationPreferences.reportUpdates'] = reportUpdates;
    if (announcements !== undefined) updatePayload['notificationPreferences.announcements'] = announcements;
    ...
    return res.status(200).json({
      ...
      data: {
        notificationPreferences: {
          pushEnabled: updatedUser.notificationPreferences?.pushEnabled ?? true,
          reportUpdates: updatedUser.notificationPreferences?.reportUpdates ?? true,
          announcements: updatedUser.notificationPreferences?.announcements ?? true
        }
      }
    });
  } catch (error) {
    console.error('Error updating notification preferences:', error);
    return res.status(500).json({ status: 'error', message: 'Failed to update notification preferences' });
  }
});
